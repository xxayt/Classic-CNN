...
class BasicBlock(nn.Module):
    # 输出shape是输入shape的1倍 (18-layers和34-layers中是相同的)
    expansion = 1

    def __init__(self, in_channel, out_channel, stride=1, downsample=None, **kwargs):
        super(BasicBlock, self).__init__()
        # stride==1时，out_channel=in_channel
...
class Bottleneck(nn.Module):
    ...
    """
    # 输出shape是输入shape的4倍 (50-layers和101-layers和152-layers中相差4倍)
    expansion = 4
...
class ResNet(nn.Module):
    ...
    def __init__(self,
                 ...
                 blocks_num,
                 num_classes=1000,
                 include_top=True,  # 为了在resnet基础上搭建更复杂网络，此处不使用，但进行了实现
                 groups=1,
                 width_per_group=64):
    ...
    # std_channel表示此conv上每个block的第一个卷积层输出的卷积核个数
    def _make_layer(self, block, std_channel, block_num, stride):
        downsample = None
        # 确定下采样层
        # 对于resnet50,101,152的conv3,4,5：需要调整特征矩阵的深度，且将高宽都缩减至一半
